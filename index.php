...
<body class="bg-gradient-to-br from-purple-900 via-blue-900 to-indigo-900 min-h-screen">
    ...
    <script>
        ...
        function initQueueUI() {
            ...
            // Habilitar acciones en todas las pestañas (quitamos bloqueo por pestaña dueña)
            enableActionsAllTabs();
        }
        ...
        function updateQueueUI(event, data, queue) {
            ...
            refreshServerJobsThrottled();
            // Banner informativo simple (sin bloqueo por pestaña)
            ...
            banner.textContent = 'Cola en servidor activa (disponible desde cualquier pestaña)';

            // Habilitar botones en todas las pestañas
            enableActionsAllTabs();
        }
                    ...
                    <div class="flex justify-between text-sm text-gray-300 mb-1">
                        ...
                        <div class="text-xs text-gray-400 flex items-center gap-2">
                            ...
                            ${(() => {
                                ...
                                const processing = job.status === 'processing' || job.status === 'queued' || job.status === 'uploading';
                                const terminal = ['completed','cancelled','error'].includes(job.status);
                                let buttons = '';
                                if (processing && !paused) {
                                    buttons += `<button class=\"bg-yellow-600/70 hover:bg-yellow-600 text-white px-2 py-0.5 rounded\" onclick=\"pauseServerJob(${job.id})\">Pausar</button>`;
                                }
                                if (paused) {
                                    buttons += `<button class=\"bg-green-600/70 hover:bg-green-600 text-white px-2 py-0.5 rounded\" onclick=\"resumeServerJob(${job.id})\">Reanudar</button>`;
                                }
                                if (terminal || paused) {
                                    buttons += `<button class=\"bg-blue-600/70 hover:bg-blue-600 text-white px-2 py-0.5 rounded\" onclick=\"retryFailedItems(${job.id})\">Reintentar fallidos</button>`;
                                }
                                if (job.status !== 'processing') {
                                    buttons += `<button class=\"bg-gray-600/70 hover:bg-gray-600 text-white px-2 py-0.5 rounded\" onclick=\"cleanupServerJob(${job.id})\">Limpiar</button>`;
                                }
                                return buttons;
                            })()}
                        </div>
        ...
        async function cleanupServerJob(jobId) {
            try {
                const fd = new FormData();
                fd.append('action','cleanup');
                fd.append('job_id', String(jobId));
                const resp = await fetch('api/jobs.php?action=cleanup', { method:'POST', body: fd });
                const data = await resp.json();
                if (!data.success) throw new Error(data.message||'');
                refreshServerJobs();
                showNotification('Trabajo limpiado', 'success');
            } catch (e) {
                showNotification('No se pudo limpiar el trabajo', 'error');
            }
        }
        async function pauseServerJob(jobId) {
            const fd = new FormData();
            fd.append('action','pause');
            fd.append('job_id', String(jobId));
            const resp = await fetch('api/jobs.php?action=pause', { method:'POST', body: fd });
            const data = await resp.json();
            if (data.success) { refreshServerJobs(); showNotification('Trabajo pausado','info'); } else { showNotification(data.message||'No se pudo pausar','error'); }
        }
        async function resumeServerJob(jobId) {
            const fd = new FormData();
            fd.append('action','resume');
            fd.append('job_id', String(jobId));
            const resp = await fetch('api/jobs.php?action=resume', { method:'POST', body: fd });
            const data = await resp.json();
            if (data.success) { refreshServerJobs(); showNotification('Trabajo encolado','success'); } else { showNotification(data.message||'No se pudo reanudar','error'); }
        }
        async function retryFailedItems(jobId) {
            const fd = new FormData();
            fd.append('action','retry_failed');
            fd.append('job_id', String(jobId));
            const resp = await fetch('api/jobs.php?action=retry_failed', { method:'POST', body: fd });
            const data = await resp.json();
            if (data.success) { refreshServerJobs(); showNotification('Reintentos encolados','success'); } else { showNotification(data.message||'No se pudo reintentar','error'); }
        }
        function enableActionsAllTabs() {
            try {
            ...
            } catch (e) { console.warn('[ENABLE_ALL_TABS] fallo', e); }
        }
